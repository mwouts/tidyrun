...
class SlurmExecutor(Executor):
    ...
    def _run_task(
        ...
        task_path = self._shared_dir / f"{task_id}.task.pickle"
        result_path = self._shared_dir / f"{task_id}.result.pickle"
        error_path = self._shared_dir / f"{task_id}.error.txt"
        stdout_path = self._shared_dir / f"{task_id}.slurm.out"
    ...
    def _run_array(
        ...
        task_id = uuid.uuid4().hex
        task_path = self._shared_dir / f"{task_id}.array.task.pickle"
        result_template = self._shared_dir / f"{task_id}.result.%a.pickle"
        error_template = self._shared_dir / f"{task_id}.error.%a.txt"
        stdout_template = self._shared_dir / f"{task_id}.slurm.%a.out"

        result_paths = [
            self._shared_dir / f"{task_id}.result.{index}.pickle"
            for index, _ in enumerate(job_ids)
        ]
        error_paths = [
            self._shared_dir / f"{task_id}.error.{index}.txt"
            for index, _ in enumerate(job_ids)
        ]
        stdout_paths = [
            self._shared_dir / f"{task_id}.slurm.{index}.out"
            for index, _ in enumerate(job_ids)
        ]
